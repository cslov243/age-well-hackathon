...
| `check_audit_hash` | **required in `record`**, refused in `check` — see below |

`check` runs **before** the vision model and answers `should_extract`. `record`
runs after it.

**`record` refuses without the `check` run's `audit_hash`.** Added 6 August 2026
...
never asked about. A duplicate record competes with the first rather than
correcting it and doubles every deadline it carries. The guarantee is worth more
than the instruction, so it is enforced where it cannot be skipped. What this
cannot enforce is the *reading*: nothing here can tell when the pages were
looked at, only that they were never asked about. A `content_hash` already present in `--records` is a no-op:
